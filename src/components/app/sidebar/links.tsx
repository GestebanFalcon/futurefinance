...
export function BigLink({ label, href, Icon, pathname }: { label: string, href: string, Icon: OverridableComponent<SvgIconTypeMap<{}, "svg">>, pathname: string }) {
    ...
    return (
        <Link
            className={`
        ${pathname === href ? "bg-emerald-100 text-emerald-600 hover:bg-emerald-200" : " text-zinc-800 hover:bg-zinc-200"} w-full flex flex-row items-center p-2 font-semibold  transition-colors rounded-sm 
        `}
            href={href}>
